...
class ComputationalModel(Base):
    def _set_latent_process(self, df_events, param_dict):
        ...
        for level1_choice,\
            ...
            if t == 1:
                level1_prob_choice2 = inv_logit( beta1*(v_hybrid[1]-v_hybrid[0]))
            else:
                level1_prob_choice2 = inv_logit( beta1*(v_hybrid[1]-v_hybrid[0]) + pi*(2*prev_level1_choice -3) )
      
            # Observe Level2 and update Level1 of the chosen option
            v_mf[level1_choice-1] += a1*(v_mf[1+ level2_choice] - v_mf[level1_choice-1])
            ...
            if level2_choice > 2:  # level2_choice = 3 or 4
                level2_prob_choice2 = inv_logit( beta2*( v_mf[5] - v_mf[4] ) )
            else : # level2_choice = 1 or 2
                level2_prob_choice2 = inv_logit( beta2*( v_mf[3] - v_mf[2] ) )
            ...
            v_mf[1+ level2_choice] += a2*(reward - v_mf[1+ level2_choice ])

            # Update Level 1 v_mf
            v_mf[level1_choice] += lambda_ * a1 * (reward - v_mf[1+level2_choice])

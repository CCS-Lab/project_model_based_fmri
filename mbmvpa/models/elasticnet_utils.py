...
def plot_elasticnet_result(save_root, 
    ...
    if isinstance(lambda_path,dict):
        lambda_path = labmda_path[list(lambda_path.keys())[0]]
    if isinstance(cv_mean_score,dict):
        cv_mean_score = np.array([np.squeeze(data) for _, dat in cv_mean_score.items()])
        cv_mean_score = cv_mean_score.reshape(-1, len(lambda_path), cv_mean_score.shape[-1])
        cv_mean_score = cv_mean_score.mean(0)
    if isinstance(cv_standard_error,dict):
        cv_standard_error = np.array([np.squeeze(data) for _, dat in cv_standard_error.items()])    
        cv_standard_error = cv_standard_error.reshape(-1, len(lambda_path), cv_standard_error.shape[-1])
        cv_standard_error = cv_standard_error.mean(0)
    
    if isinstance(lambda_val, dict):
        lambda_val = np.array([np.squeeze(data) for _, dat in lambda_val.items()])
        
    if isinstance(coef_paths,dict):
        coef_path = np.array([np.squeeze(data) for _, dat in coef_path.items()])    
    ...
    plt.axvspan(lambda_val.min(), lambda_val.max(),

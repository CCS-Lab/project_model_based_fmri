...
class MVPA_CV():
    ...
    ----------
    X_dict : dict
        Dictionary of multi-voxel signals indexed by the subject ID.
        X_dict[{subject_id}] = np.array(time, voxel_num)

    y_dict : dict
        Dictionary of latent process indexed by the subject ID.
    ...
    model_param_dict : dict, default={}
        ...
        which would be required in configuring models if applicable.
        (It is not used in models offered by this package.)
    ...
    n_cv_repaet : int, default=1
        ...
        Bigger the value, more time for computation and more stability.
        (recommend to use smaller than 5 or it will need much time to finish.)
    ...
    fit_reporter : utils.report.FitReporter
        ...
        Please refer to the documentation of ``report`` module.

    post_reporter : utils.report.PostReporter
        Reporting object which will make reports after all folds are done.
        Results like brain activation map, or goodness-of-fit plot 
        can be generated by PostReporter. 
        Please refer to the documentation of ``report`` module.
    ...
    def run(self,**kwargs):
        ...
        r"""Run MVPA cross-validation experiment (Fitting & Interpreting).
        """
